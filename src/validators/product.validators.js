...
const createProductValidation = [
  body('name').trim().escape().isLength({ min: 2, max: 200 }).withMessage('Name 2-200 chars'),
  body('description').optional().trim().escape().isLength({ max: 5000 }).withMessage('Desc too long'),
  ...
  body('category_id').optional().isInt({ min: 1 }).withMessage('category_id must be positive'),
];
