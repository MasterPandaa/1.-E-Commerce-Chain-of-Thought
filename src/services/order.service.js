...
class OrderService {
  async checkout(user_id, shipping) {
    ...
    for (const it of items) {
      ...
      if (!prod) throw ApiError.badRequest(`Product not found: ${it.product_id}`);
      if (prod.stock < it.quantity) throw ApiError.badRequest(`Insufficient stock for ${prod.name}`);
    }
    ...
    try {
      ...
      for (const it of items) {
        ...
        if (decRes.affectedRows === 0) throw ApiError.badRequest('Stock changed, please try again');
        ...
        await conn.execute(
          `INSERT INTO order_items (order_id, product_id, product_name, unit_price_cents, quantity, line_total_cents)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [order_id, it.product_id, it.name, it.price_cents, it.quantity, line_total_cents]
        );
      }
  ...
  async listUserOrders(user_id, { page = 1, limit = 20 }) {
    const { rows, count } = await OrderModel.listByUser(user_id, { page, limit });
    return { items: rows, page, limit, total: count, pages: Math.ceil(count / limit) };
  }

  async getUserOrder(user_id, id) {
    const order = await OrderModel.findById(id);
    if (!order || order.user_id !== user_id) throw ApiError.notFound('Order not found');
