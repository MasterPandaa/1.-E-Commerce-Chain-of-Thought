...
class CartModel {
  static async getOrCreateByUserId(user_id) {
    ...
    const [res] = await db.execute("INSERT INTO carts (user_id) VALUES (?)", [
      user_id,
    ]);
  ...
  static async insertItem({ cart_id, product_id, quantity, price_cents }) {
    const sql =
      "INSERT INTO cart_items (cart_id, product_id, quantity, price_cents) VALUES (?, ?, ?, ?)";
  ...
  static async updateItemQuantity({
    ...
    const sql =
      "UPDATE cart_items SET quantity = ?, price_cents = ? WHERE cart_id = ? AND product_id = ?";
    const [res] = await db.execute(sql, [
      quantity,
      price_cents,
      cart_id,
