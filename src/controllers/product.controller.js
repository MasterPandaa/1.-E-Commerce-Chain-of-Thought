...
exports.update = [async (req, res, next) => {
  try {
    const prod = await productService.update(parseInt(req.params.id, 10), req.body);
...
exports.list = [async (req, res, next) => {
  try {
    const data = await productService.list({
      ...
      min_price: req.query.min_price ? parseInt(req.query.min_price, 10) : undefined,
...
exports.addImage = [async (req, res, next) => {
  try {
    ...
    const images = await productService.addImage(parseInt(req.params.id, 10), `/uploads/${req.file.filename}`);
