...
class ProductModel {
  static async create({ name, slug, description, price_cents, currency, stock, category_id }) {
    const sql = `INSERT INTO products (name, slug, description, price_cents, currency, stock, category_id)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`;
    const [res] = await db.execute(sql, [name, slug, description, price_cents, currency, stock, category_id]);
    return res.insertId;
  }

  static async update(id, fields) {
    const allowed = ['name', 'slug', 'description', 'price_cents', 'currency', 'stock', 'category_id'];
  ...
  static async list({ page = 1, limit = 20, category_id, min_price, max_price, q }) {
    const offset = (page - 1) * limit;
    const where = [];
    const params = [];

    if (category_id) { where.push('p.category_id = ?'); params.push(category_id); }
    if (min_price != null) { where.push('p.price_cents >= ?'); params.push(min_price); }
    if (max_price != null) { where.push('p.price_cents <= ?'); params.push(max_price); }
