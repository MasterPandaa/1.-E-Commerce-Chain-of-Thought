...
function generateInvoice({ order, items }) {
  ...
  doc.text(`${order.shipping_city}, ${order.shipping_state} ${order.shipping_postal_code}`);
  ...
  items.forEach((it) => {
    ...
    doc.text((it.unit_price_cents / 100).toFixed(2) + ' ' + order.currency, 350, doc.y, { continued: true });
    doc.text((it.line_total_cents / 100).toFixed(2) + ' ' + order.currency, 420);
  });

  doc.moveDown();
  doc.text(`Subtotal: ${(order.subtotal_cents / 100).toFixed(2)} ${order.currency}`, { align: 'right' });
  doc.text(`Shipping: ${(order.shipping_cents / 100).toFixed(2)} ${order.currency}`, { align: 'right' });
  doc.font('Helvetica-Bold');
  doc.text(`Total: ${(order.total_cents / 100).toFixed(2)} ${order.currency}`, { align: 'right' });
