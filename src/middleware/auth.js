...
function auth(required = true) {
  return async (req, res, next) => {
    try {
      ...
      if (!token) {
        if (required) return next(ApiError.unauthorized('Missing authorization token'));
        req.user = null;
        return next();
      }
...
function requireRole(role) {
  return (req, res, next) => {
    ...
    if (req.user.role !== role) return next(ApiError.forbidden('Insufficient permissions'));
    next();
  };
}
